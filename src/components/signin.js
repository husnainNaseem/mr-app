  ...
  const loginWithGoogle = () => {
    ...
    signInWithPopup(auth, new GoogleAuthProvider())
      .then(async (result) => {
        ...
        console.log("current user is here", user);
        // Add a new document with a generated id.
        // console.log("db======", db);
        // await setDoc(doc(db, "users", "new-user-id"), user);
        ...
        updateProfile(auth.currentUser, {
          ...
          .then(function() {
            ...
            setDoc(usersRef, { capital: true }, { merge: true });
  ...
  const loginWithGithub = () => {
    ...
    signInWithPopup(auth, new GithubAuthProvider())
      .then((result) => {
        ...
        window.location.href = "/doctors/appointments";
      })
      .catch((error) => {
        setErrorMessage(error.message);
      });
  ...
  return (
    <div className="flex justify-center items-center min-h-screen absolute inset-0 py-12 mt-0 ">
      ...
      >
        ...
        <button
          type="submit"
          className="mt-8 text-sm font-semibold leading-none text-white w-full px-40 py-4 bg-[#00B3E4] rounded-lg mt-8 focus:outline-none"
        >
          Log in
        </button>
        {manualErrorMessage && (
